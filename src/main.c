...
#pragma GCC diagnostic ignored "-Woverride-init"
  ...
  static void *disp[TOK_CAPACITY] = {
    [TOK_NONE]        = &&TOK_NONE,
    [1 ... TOK_COUNT] = &&TOK_ALL,
    [TOK_WHITE_RANGE] = &&TOK_WHITESPACE,
    [TOK_SPACE]       = &&TOK_WHITESPACE,
...
#pragma GCC diagnostic pop // ignored "-Woverride-init"
  ...
  FrieEntry: {
    ...
    cKey   =  pText[iText];
    cText  =  cKey;
    iLen   =  MAXIMAL_MUNCH_LEN;
    pEntry = &pRoot->ascii[cKey].len[iLen];
  ...
  FrieNextChar: {
    ...
    if ((cFrie == TOK_DELIM) & IS_DELIM_CHAR(cText)) {
      fprintf(stderr, "match \n");
      prevmatch = true;
      prevdelim = true;
      cFrie     = pFrie[++iFrie];
